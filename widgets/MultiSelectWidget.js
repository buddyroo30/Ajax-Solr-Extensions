...
AjaxSolr.MultiSelectWidget = AjaxSolr.AbstractFacetWidget.extend({
  ...
  afterRequest: function () {
    ...
    $('#' + ac_id).autocomplete({
	 ...
	 var check_state = self.check_state;
	 if (typeof check_state == 'undefined') { self.check_state = {}; check_state = self.check_state; }
	 if (this.checked) { check_state[facet] = true; } else { delete check_state[facet]; }
	 var checked_facets_arr = $.map(check_state, function(v,i) { return '"' + i + '"'; });
	 self.manager.store.removeByValue('fq' ,new RegExp('^' + self.field));
	 if (checked_facets_arr.length > 0) {
	     var solr_query;
  ...
  //				  var sel_facet = ui.item.value;
  //				  $('#' + ac_id).val('');


    //console.log(this.field);
    //console.log(checked_objectedItems);
    //console.log(unchecked_objectedItems);
...
//		 matches_arr[i] = matches_arr[i] + ' - ' + field_facet_counts[matches_arr[i]];
//	     }

       //var solr_field_query = '(*' + search_term.toUpperCase() + '* OR *' + search_term + '* OR *' + search_term.toLowerCase() + '*)';
       //var search_string = 'rows=0&facet=true&q=' + self.field + ':' + solr_field_query + '&facet.field=' + self.field + '&facet.mincount=1&facet.threads=-1&json.nl=map';

		                     //source: this.init_ac_values,

// else if (sort_type == 'init_count') { //the count of the facet in the full result set (with no queries or filter queries applied, i.e. *:*)
//        objectedItems.sort(function (a, b) {
//          return b.init_count < a.init_count ? -1 : 1;
//        });
//    }

    //    	     var ac_id = '#' + self.field + '_all_extra';
    //    	     $(ac_id).autocomplete( "option", "appendTo", self.field + '_container');
